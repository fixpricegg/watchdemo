...
def analyze_demo(demo_path, player):
    ...
    if deaths.empty:
        raise ValueError(
            "В демке не найдено событий player_death."
        )
    ...
    if "weapon" in deaths.columns:
        deaths = deaths[
            ~deaths["weapon"].isin(KNIFE_WEAPONS)
        ]

    if deaths.empty:
        raise ValueError(
            "После очистки не осталось игровых смертей."
        )
    ...
    if deaths_clean.empty:
        raise ValueError(
            "После фильтрации по live-раундам не осталось смертей."
        )
        ...
        events.append(
            create_event(
                ...
                description=(
                    ...
                    f"{missed_trade['killer_alive_ticks'] / TICKRATE:.1f} сек."
                ),
                data=missed_trade
            )
        )
